...
public:
    ...
    {
        Type mag = magnitude();
        if (mag != 0.0) *this = this->scale(Type(1.0)/mag);
    }
    ...
    void toCartesian(Type *xyz) {
        xyz[0] = 1.0f - 2.0f*y*y - 2.0f*z*z;
        xyz[1] = 2.0f*x*y + 2.0f*w*z;
        xyz[2] = 2.0f*x*z - 2.0f*w*y;
    }
    ...
    void fromYPR(Type *ypr) {
        ...
        //MNEMONIC: swivel on your swivel chair, look up/down, then tilt your head left/right...
        //           ... thats how we yaw, pitch'n'roll.
        Type t0 = cos(ypr[0] * 0.5);
        Type t1 = sin(ypr[0] * 0.5);
        Type t2 = cos(ypr[2] * 0.5);
        Type t3 = sin(ypr[2] * 0.5);
        Type t4 = cos(ypr[1] * 0.5);
        Type t5 = sin(ypr[1] * 0.5);
